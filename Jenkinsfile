...
node 
{
   ...
   if ( """${create_infra}""" == true)
   {
      ...
      {
         build job: 'infrastructure_pipeline', parameters: [string(name: 'environment', value: 'test'), string(name: 'branch', value: 'test_gateway')]
	   }
   }
   ...
   stage('start CD job')
   {
      ...
      build job: '/cd_pipeline/prod_gateway', propagate: false, wait: false
   }
